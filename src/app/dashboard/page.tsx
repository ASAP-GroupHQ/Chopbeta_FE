  ...
  return (
    <LayoutWrapper>
      <div className="space-y-8">
        {/* UPPER HEADER STRIP: Handles dynamic text layout on left and desktop actions on right */}
        <div className="flex items-center justify-between w-full">
          {/* Left Side: Dynamic Greetings block */}
          ...
          {/* Right Side: Desktop Utility Suite (Hidden on mobile viewports up to large screen breakage) */}
          <div className="hidden lg:flex items-center gap-3">
            {/* Help Button Box */}
            <button className="w-10 h-10 flex items-center justify-center bg-white border border-gray-100 rounded-xl shadow-[0_2px_6px_rgba(0,0,0,0.02)] text-[#1A1A2E] hover:bg-gray-50 hover:text-green-800 transition-all active:scale-95 cursor-pointer">
              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
              ...
              >
                <g clipPath="url(#clip0_470_967)">
                  ...
                  <rect
                    ...
                    height="5"
                    rx="2.5"
                    fill="#E85D26" // Updated to match your custom brand orange exactly!
                  />
                </g>
                <defs>
            ...
            {/* Integrated Profile Card Pill Container */}
            <button className="h-10 flex items-center gap-2 bg-white border border-gray-100 rounded-xl pl-1.5 pr-3 shadow-[0_2px_6px_rgba(0,0,0,0.02)] hover:bg-gray-50 transition-all active:scale-[0.99] cursor-pointer">
              {/* Profile Image Box */}
              ...
              {/* Name String */}
              <span className="text-sm font-bold text-[#1A2E35] ml-0.5">Victor</span>
              
              {/* Dropdown Accent Chevron */}
              <FiChevronDown className="w-4 h-4 text-gray-500 stroke-[2.5]" />
            </button>
          </div>
        </div>

        {/* Core Canvas Action items */}
        <HeroBanner />

        {/* Next up: We will drop the QuickActions Grid & QuickMeals Carousel panels here! */}
      </div>
    </LayoutWrapper>
  );
}