...
export default function HeaderActions() {
  return (
    <div className="hidden lg:flex items-center gap-3">
      ...
      <button
        type="button"
        className="w-10 h-10 flex items-center justify-center bg-white border border-gray-100 rounded-xl shadow-[0_2px_6px_rgba(0,0,0,0.02)] text-[#1A2E35] hover:bg-gray-50 transition-all cursor-pointer"
      ...
      <button
        type="button"
        className="h-10 flex items-center gap-2 bg-white border border-gray-100 rounded-xl pl-1.5 pr-3 shadow-[0_2px_6px_rgba(0,0,0,0.02)] hover:bg-gray-50 transition-all cursor-pointer"
      >
        <div className="relative w-7 h-7 rounded-lg overflow-hidden border border-gray-100 select-none">
          <Image
            src="/images/team/emmanuel_ozo.JPG"
            alt="User profile avatar context trigger"
            fill
            sizes="28px"
        ...
        <span className="text-sm font-bold text-[#1A2E35] ml-0.5 select-none">
          Victor
        </span>
        <FiChevronDown className="w-4 h-4 text-gray-400 stroke-[2.5] flex-shrink-0" />
      </button>
    </div>
  );
}
